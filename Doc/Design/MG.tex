...
\begin{description}
\item [\refstepcounter{acnum} \actheacnum \label{acSkipTurn}:] \sout{Implementation of a Skip Turn button. This would prevent deadlock in the game if a player is unable to make a move. } \textcolor{red}{Change has been implemented}
\item [\refstepcounter{acnum} \actheacnum \label{acExchangeTile}:] \sout{Implementation of an Exchange Tiles button. This would help prevent deadlock if available tiles do not create a word that can be placed.} \textcolor{red}{Change has been implemented}
...
\begin{description}
\item [\refstepcounter{mnum} \mthemnum \label{mHHGW}:] \sout{Game Window} \textcolor{red}{Main Game} %Hardware-Hiding Module
\item [\refstepcounter{mnum} \mthemnum \label{mBHGB}:] \sout{Gameboard Boolean Checks} \textcolor{red}{BoardChecks} % Software Decision 
\item [\refstepcounter{mnum} \mthemnum \label{mBHET}:] EndTurn \sout{Boolean Checks} % Software Decision
\item [\refstepcounter{mnum} \mthemnum \label{mBHVW}:] \sout{Valid Word Boolean Checks} WordChecks % % Software Decision
\item [\refstepcounter{mnum} \mthemnum \label{mBHGV}:] \sout{Game View} \textcolor{red}{GameController} %Behaviour-Hiding Module
...
\item [\refstepcounter{mnum} \mthemnum \label{mSDB}:] Bag % Software Decision
 \item [\refstepcounter{mnum} \mthemnum \label{mSDB}:] \textcolor{red}{WidgetCreation} %Behviour Hiding
\end{description}
...
\textcolor{red}{GameController}    & \textcolor{red}{FR6, FR7, FR8, FR9, FR10, FR12, PR2, MS1, MS2, MS3}                                                                                                                   \\ \hline
\sout{MainGame} \textcolor{red}{Main} & \begin{tabular}[c]{@{}l@{}}FR2, FR4, FR7, \sout{FR8}, FR10, \sout{FR12} \textcolor{red}{FR13, FR15, FR16, FR17}\\ LF1, UH1, UH2, UH3, UH4, UH5, UH7,\\  PR1, PR2, OE1, OE2, CR1, LR1, HSR1\end{tabular} \\ \hline
\end{tabular}
...
The purpose of having modules is to decompose the system into manageable pieces \textcolor{red}{of code} that can be more easily developed \textcolor{red}{and maintained} \sout{than} \textcolor{red}{as compared to} a monolithic piece of software. The way \textcolor{red}{the} modules are decomposed is based around the principle of information hiding, which states that each module has one secret that is hidden from the other modules. These secrets are parts of the implementation that are likely to change. Each module also has a service which is the functionality that the module provides without describing the implementation of these services. For each of the modules given in the hierarchy their secret and service is listed as well as how the module will be implemented.

\subsection{Hardware Hiding Modules - \textcolor{red}{Main} Game (\mref{mHHGW})} 
\begin{description}
...
%When I tried the \sout command within the subsection the latex editor gave an error. 
\begin{description}
...
\midrule
\acref{acSkipTurn} & \sout {\mref{mBHGV}} \textcolor{red}{Change implemented}\\ %game view
\acref{acExchangeTile} & \sout{\mref{mBHGV}} \textcolor{red}{Change implemented}\\
& \sout{\mref{mBHET}} \textcolor{red}{Change implemented}\\ %exchange tile 
